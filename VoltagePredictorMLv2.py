# extension of VoltagePredictorML where the classifier output also depends on how many the dv_dt crosses the predefined threshold
# also lists all the voltage unstable cases
import pickle
import numpy as np
import os
...
for key in VoltageDataDict:
	...
	# get the dv_dt
	dv_dt = []
	for i in range(len(steadyStateV)):
		try:
			diff = abs((steadyStateV[i]-steadyStateV[i-1])/timestep)
			dv_dt.append(diff)
		except: # when  i=0, since there is no i-1
			continue

	# get how many times the voltage or the dv_dt exceed the thresholds
	abnormalVList = [v for v in steadyStateV if v< 0.9 or v> 1.1]
	abnormaldVdTList = [dv for dv in dv_dt if dv > 0.2]

	if len(abnormalVList) > 10 or len(abnormaldVdTList) > 3:
		classVec[k] = 1
...
for x in np.linspace(0.01,0.5,50):

	ClassWeightDict = {}
	ClassWeightDict[1] = 1.0
	ClassWeightDict[0] = x
	...
	print 'Unstable to stable weight ratio: ', 1.0/x
...
SVMStart = time.time()
model_SVM = svm.SVC(class_weight={0:0.01, 1:1.0})
...
with open('UnstableCaseList.txt','w') as f:
	for line in unstableVID:
		f.write(line)
		f.write('\n')
"""