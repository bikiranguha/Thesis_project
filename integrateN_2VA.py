# script to get the voltage and angle data for the transient part (just after the last event) as  well as the steady part
# integrate all the voltage and angle data for the large scale N-2 F scenarios
# all the generator trips are now included
# any cases where there was originally angle separation but generator trips caused convergence issues, were ignored
from getBusDataFn import getBusData
import csv
import pandas as pd

"""
#### get the steady state samples (voltage and angle)
...
def generateEventDict(readerfile,eventlist,numSamples,skipset,eventdict):
...
eventdictv, exploredsetv = generateEventDict(vreaderfile,eventlist,120,exploredsetv,eventdictv)
eventdicta, exploredseta = generateEventDict(areaderfile,eventlist,120,exploredseta,eventdicta)
...
for i in range(1,10):
    ...
    eventdictv, exploredsetv = generateEventDict(vreaderfile,currentEventList,120,exploredsetv,eventdictv)
    eventdicta, exploredseta = generateEventDict(areaderfile,currentEventList,120,exploredseta,eventdicta)
...
vfileNew = 'obj/vN_2FNewSteady.csv'
vfileObj = open(vfileNew,'wb')
eventWriterv = csv.writer(vfileObj)

afileNew = 'obj/aN_2FNewSteady.csv'
...
with open('obj/eventN_2FNew.txt','w') as f:
    for event in organizedeventlist:
        f.write(event)
        f.write('\n')
...
with open('obj/buslistN_2F.csv','w') as f:
    f.write(buslistStr)
...
vfileNew = 'obj/vN_2FNewTransient.csv'
vfileObj = open(vfileNew,'wb')
eventWriterv = csv.writer(vfileObj)

afileNew = 'obj/aN_2FNewTransient.csv'
afileObj = open(afileNew,'wb')
eventWritera = csv.writer(afileObj)
