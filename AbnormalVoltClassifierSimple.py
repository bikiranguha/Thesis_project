# get the voltage classifications
# plot distributions of the following parameters wrt class 0 (normal voltage) and class 1 (low voltage):
...
for key in VoltageDataDict:
    ...
    #abnormalVList = [steadyV[j] for j in range(steadyV.shape[0]) if (steadyV[j] < 0.95*pre_cont_volt or steadyV[j] > 1.1) and dv_dt[j] < 0.01]
    ...
    if len(abnormalVList) > 10: # class 1
...
with open('PerformanceVAbs.txt','w') as f:
    ...
    f.write('\n')
    f.write('negative: voltage within constraints, positive: voltage outside constraints')
    f.write('\n')
