# plot all the  cases in a file 
#UnstableCaseFile = 'UnstableCaseList.txt'
AbnormalVCaseFile = 'AbnormalVList.txt'

with open(AbnormalVCaseFile,'r') as f:
...
for line in fileLines:
    if line == '':
        continue
    abnormalCases.append(line)

import os
currentdir = os.getcwd()
PlotDir = currentdir + '/AbnormalVPlots'
...
k = 0
for line in abnormalCases:
    if line == '':
        continue
    words = line.split('/')
    event = words[0].strip()
    Bus = words[1].strip()
    voltageValues = VoltageDataDict[line]
