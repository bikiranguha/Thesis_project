# test an event classifier where the events to be classified are: (balanced and unbalanced) faults, generator outages (line outages are ignored)
# each sample's time shifted copies are generated and put into the input array
...
###
timesteps = 40
std = 0.001 # noise standard deviation
numcyc = 2
#shiftRange = timesteps/2
shiftRange = 1 # this basically does no shift
start = 0.1
...
for idx, row in enumerate(readerV):
    ...
    out = avgFilter(row,numcyc)
    # add some noise to the outputs
    out = np.array(out) + np.random.normal(0,std,len(out)) # normal noise with standard deviation of std
...
for idx, row in enumerate(vReader):
    ...
    for t in range(shiftRange): # time shift
        key = '{}S{}'.format(eventID,t)
        if key not in N_1EventDict:
            N_1EventDict[key] = []
        ...
        for bs in range(3): # simulate three phase
            for v in row[startind-t:endind-t]:
                
                N_1EventDict[key].append(v)
...
"""
####### 
# line outage data (without any time shifts)
lineoutvdata = []
lineOutDir = 'LineOut'
...
# plot any event
#eventID  = 'R100/F3003/ABCG/8.0e-03' # high impedance fault
eventID  = 'R100/G211S0' # generator outage
eventInd = eventTracker.index(eventID)
sampleData = fullArray[eventInd]
plt.plot(sampleData[:timesteps])
plt.title(eventID)
plt.grid()
plt.show()
...
kfold = StratifiedKFold(n_splits=5, shuffle=True) # preserve the distribution of classes in folds
cvscores = []
for train, test in kfold.split(x_train, y_train):
    ...
    #model.add(LSTM(10, input_shape=(x_train.shape[1], x_train.shape[2]))) # the number refers of timesteps in a sample
...
# convert the output of LSTM to a vector, so that we can get a confusion metrix
ylabels_pred =[] # list where the prob values are converted to class labels
for i in range(y_test_pred.shape[0]):
    r = y_test_pred[i].argmax()
    ylabels_pred.append(r)
ylabels_pred  = np.array(ylabels_pred)

# get the confusion matrix
conf_mat = confusion_matrix(y_test, ylabels_pred)
print (conf_mat)



# Experiments to carry out:
# add positive timeshifts as well
# implement a callback function which stops when increasing epochs doesnt do much
